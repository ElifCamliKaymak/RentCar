...
using RentCar.ViewModels.CarDescriptionVms;
...
namespace RentCar.WebUI.ViewComponents.CarDetailsViewComponents
{
    public class _CarDetailsCarDescriptionByCarIdComponentPartial : ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync(int id)
        {
            ...
            var responseMessage = await client.GetAsync($"https://localhost:7263/api/CarDescriptions/CarDescriptionByCar/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<List<ResultCarDescriptionVM>>(jsonData);
                return View(values);
            }
            return View();
        }
